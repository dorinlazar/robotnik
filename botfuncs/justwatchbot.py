...
class JustWatchBot(MessageHandler):
    ...
    def on_message(self, msg):
        ...
        if len(spl[0]) == 2 and spl[0].upper() == spl[0]:
            country = spl[0]
            msg = " ".join(spl[1:])
        return self.on_country_message(msg, country)

    def on_country_message(self, msg: str, country: str):
        just_watch = JustWatch(country=country)
        jw_results = just_watch.search_for_item(query=msg)
        if (
            type(jw_results) != dict
            or "items" not in jw_results
            or len(jw_results["items"]) == 0
        ):
    ...
    def to_info_display(self, info, msg):
        ...
        if "offers" in info:
            urls = list(
                set(["<" + u["urls"]["standard_web"] + ">" for u in info["offers"]])
            )
        imdb_text = ""
        if "scoring" in info:
        ...
        if "title" in info and "original_release_year" in info:
            title = (
                ...
                + info["title"]
                + "("
                + str(info["original_release_year"])
                + ")]"
            )
